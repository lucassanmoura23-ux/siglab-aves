...
export const AIReports: React.FC<AIReportsProps> = ({ records }) => {
  ...
  const handleDownloadPDF = () => {
    ...
    // @ts-ignore - html2pdf is loaded via CDN globally
      ...
      <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <FilterSelect 
            ...
            onChange={setFortnightFilter} 
            options={['-- Por Quinzena --', ...fortnightOptions.map(o => o.value)]} 
            customLabels={fortnightOptions}
          />
          ...
          <button 
            ...
            className="text-[10px] font-black text-blue-600 uppercase hover:underline ml-2"
          >
            Limpar
          </button>

          <div className="ml-auto text-[10px] font-black text-amber-600 bg-amber-50 px-3 py-1.5 rounded-lg flex items-center gap-2">
            <BarChart2 size={14} />
            {filteredData.length} DIAS ANALISADOS
          </div>
        </div>
      </div>
      ...
      <div className="min-h-[300px]">
        {isGenerating ? (
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden animate-in slide-in-from-top-4">
             <div className="px-6 py-3 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
                <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-2">
                   <FileText size={14} /> Resumo Técnico Gerado
                </span>
                <div className="flex items-center gap-4">
                  <button onClick={handleDownloadMarkdown} className="text-amber-600 hover:text-amber-700 font-black text-[10px] uppercase tracking-widest flex items-center gap-1 transition-colors">
                    <Download size={14} /> Markdown
                  </button>
                  <button onClick={handleDownloadPDF} className="text-blue-600 hover:text-blue-700 font-black text-[10px] uppercase tracking-widest flex items-center gap-1 transition-colors">
                    <FileDown size={14} /> Baixar PDF
                  </button>
                </div>
             </div>
             <div className="p-8 prose prose-amber max-w-none" ref={reportRef}>
                {/* PDF Header (only visible on PDF or styled specifically) */}
                <div className="hidden pdf-only flex justify-between items-center mb-6 border-b pb-4">
                   <h1 className="text-2xl font-black text-gray-900 m-0">SIGLAB AVIÁRIO</h1>
                   <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Relatório Técnico</span>
                </div>
                ...
                <div className="hidden pdf-only mt-10 pt-4 border-t text-[10px] text-gray-400 flex justify-between uppercase font-black tracking-widest">
                   <span>Gerado por SIGLAB IA</span>
          ...
          <div className="bg-white rounded-2xl border border-dashed border-gray-200 p-16 flex flex-col items-center justify-center text-center">
             ...
             <p className="text-gray-400 text-xs font-black uppercase tracking-widest">Clique no botão acima para iniciar a análise rápida</p>
          </div>
        )}
      </div>

      <div className="flex items-start gap-3 p-4 bg-amber-50/30 rounded-xl border border-amber-100">
         <AlertCircle className="text-amber-500 shrink-0 mt-0.5" size={16} />
         <p className="text-[10px] font-bold text-amber-800 leading-tight">
           O relatório agora prioriza as **Observações** inseridas manualmente para justificar variações de postura e mortalidade.
         </p>
      </div>
...
const FilterSelect = ({ value, onChange, options, customLabels }: { value: string, onChange: (v: string) => void, options: string[], customLabels?: { label: string, value: string }[] }) => (
  <div className="relative">
    <select 
      ...
      className="appearance-none bg-gray-50 border border-gray-100 text-[10px] font-black py-2 pl-3 pr-8 rounded-lg outline-none focus:ring-2 focus:ring-amber-100 transition-all cursor-pointer uppercase tracking-tight"
    >
      {options.map(o => {
