...
export const GeneralDashboard: React.FC<GeneralDashboardProps> = ({ records, batchRecords }) => {
  ...
  const fortnightOptions = useMemo(() => {
    ...
    records.forEach(r => { const [y, m] = r.date.split('-'); uniquePeriods.add(`${y}-${parseInt(m) - 1}`); });
  ...
  const renderMaturityChart = () => {
    ...
    return (
      <div className="relative w-full h-full flex flex-col">
        <div className="flex items-center gap-3 mb-4"><div className="w-2 h-5 bg-blue-600 rounded-full"></div><h3 className="text-sm font-black text-gray-700 uppercase tracking-tight">Curva de Maturidade do Lote (Postura % x Idade)</h3></div>
    ...
    return (
      <div className="relative w-full h-full group">
        <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full overflow-visible">
          ...
          {MONTHS_SHORT.map((m, i) => (<text key={m} x={getX(i)} y={height - 20} textAnchor="middle" className="text-[10px] fill-gray-400 font-bold uppercase">{m}</text>))}
          <path d={d} fill="none" stroke={lineColor} strokeWidth="4" strokeLinecap="round" className="animate-draw" />
  ...
  return (
    <div className="space-y-6">
      ...
      {!stats ? <div className="p-20 text-center bg-white rounded-2xl border border-dashed border-gray-200 text-gray-400 font-medium">Nenhum dado encontrado para os filtros selecionados.</div> : (<div className="space-y-6 animate-in fade-in duration-500"><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4"><KPICard title="Total Ovos" value={stats.totalEggs.toLocaleString('pt-BR')} subtext="(acumulado no período)" icon={<Egg />} color="blue" /><KPICard title="Taxa Média Postura" value={`${stats.avgLayingRate.toFixed(1)}%`} subtext="(produção/população)" icon={<TrendingUp />} color="green" /><KPICard title="Aves Vivas" value={stats.currentBirds.toLocaleString('pt-BR')} subtext="(saldo atual somado)" icon={<Bird />} color="purple" /><KPICard title="Mortalidade" value={`${stats.totalMortality} (${stats.mortalityRate.toFixed(2)}%)`} subtext="Total e Taxa no Período" icon={<AlertTriangle />} color="red" /></div><div className="grid grid-cols-1 lg:grid-cols-3 gap-6"><div className="lg:col-span-2 bg-white p-8 rounded-2xl border border-gray-100 shadow-sm"><div className="flex items-center justify-between mb-8"><h3 className="text-sm font-black text-gray-700 uppercase tracking-widest flex items-center gap-2"><div className="w-2 h-5 bg-indigo-600 rounded-full"></div> Qualidade dos Ovos</h3></div><div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center"><div className="relative flex justify-center"><svg viewBox="0 0 100 100" className="w-48 h-48 -rotate-90"><circle cx="50" cy="50" r="40" fill="transparent" stroke="#f1f5f9" strokeWidth="12" />{qualitySegments.map((seg, idx) => (<circle key={idx} cx="50" cy="50" r="40" fill="transparent" stroke={seg.color} strokeWidth="12" strokeDasharray={`${seg.dash} 251.32`} strokeDashoffset={seg.offset} className="transition-all duration-1000 ease-out" />))}</svg><div className="absolute inset-0 flex flex-col items-center justify-center text-center"><p className="text-3xl font-black text-gray-800 ladies-none">{((stats.quality.clean / (stats.quality.total || 1)) * 100).toFixed(0)}%</p><p className="text-[8px] font-black text-gray-400 uppercase tracking-widest mt-1">Limpos</p></div></div><div className="grid grid-cols-2 gap-3"><QualityBox label="Limpos" value={stats.quality.clean} total={stats.quality.total} color="emerald" /><QualityBox label="Sujos" value={stats.quality.dirty} total={stats.quality.total} color="orange" /><QualityBox label="Trincados" value={stats.quality.cracked} total={stats.quality.total} color="red" /><QualityBox label="Cama" value={stats.quality.floor} total={stats.quality.total} color="purple" /></div></div></div><div className="space-y-4"><KPICard title="Peso Médio Ovos" value={`${stats.avgEggWeight.toFixed(1)}g`} subtext="" icon={<Scale />} color="teal" /><KPICard title="Peso Médio Aves" value={`${stats.avgBirdWeight.toFixed(1)}g`} subtext="" icon={<Weight />} color="indigo" /><div className="bg-emerald-600 p-6 rounded-2xl shadow-lg shadow-emerald-100 flex items-center gap-4 text-white"><div className="p-3 bg-white/20 rounded-xl"><CheckCircle2 size={24} /></div><div><p className="text-[10px] font-bold uppercase tracking-widest opacity-80">Status Sanitário</p><p className="text-lg font-black">Lote Saudável</p></div></div></div></div><div className="space-y-6"><div className="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm w-full"><div className="h-72 w-full">{renderLineChart()}</div></div><div className="bg-white px-8 pt-4 pb-12 rounded-2xl border border-gray-100 shadow-sm w-full"><div className="w-full h-[400px]">{renderMaturityChart()}</div></div></div></div>)}
    </div>
  );
};
