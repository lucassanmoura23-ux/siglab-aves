...
export const GeneralDashboard: React.FC<GeneralDashboardProps> = ({ records, batchRecords }) => {
  ...
  const filteredRecords = useMemo(() => {
    return records.filter(record => {
      ...
      // Fixed: Define recordDate to prevent "Cannot find name 'recordDate'" error
  ...
  const renderMaturityChart = () => {
    const width = 900; const height = 300; const paddingLeft = 60; const paddingRight = 30; const paddingTop = 10; const paddingBottom = 50;
    ...
    return (
      <div className="relative w-full overflow-visible">
        <div className="flex justify-center flex-wrap gap-x-4 gap-y-1 mb-4">{maturityData.map((b, i) => (<div key={b.batchId} className="flex items-center gap-1.5"><div className="w-2.5 h-0.5 rounded-full" style={{ backgroundColor: lineColors[i % lineColors.length] }}></div><span className="text-[9px] font-black uppercase tracking-wider text-gray-500">Lote: {b.batchId}</span></div>))}</div>
        <svg viewBox={`0 0 ${width} ${height}`} className="w-full block overflow-visible" preserveAspectRatio="xMidYMid meet">
          {[0, 25, 50, 75, 100].map(v => (<g key={v}><line x1={paddingLeft} y1={getY(v)} x2={width - paddingRight} y2={getY(v)} stroke="#f1f5f9" strokeWidth="1" strokeDasharray="4 4" /><text x={paddingLeft - 10} y={getY(v) + 4} textAnchor="end" className="text-[10px] fill-gray-400 font-bold">{v}%</text></g>))}
          ...
          {[0, 20, 40, 60, 80, 100].filter(a => a <= maxAge).map(age => (<text key={age} x={getX(age)} y={height - 25} textAnchor="middle" className="text-[11px] fill-gray-800 font-black">{age}</text>))}
          <text x={width / 2} y={height - 5} textAnchor="middle" className="text-[10px] font-black fill-gray-400 tracking-tight uppercase">Idade (Semanas)</text>
        ...
        {hoveredAge !== null && (<div className="absolute bg-white border border-gray-200 p-2 rounded-lg shadow-xl z-50 pointer-events-none min-w-[120px]" style={{ top: '0px', left: `${Math.min(80, Math.max(10, (getX(hoveredAge) / width) * 100))}%` }}><div className="text-[9px] font-black text-gray-400 mb-1 border-b pb-1 uppercase">{hoveredAge} Semanas</div>{maturityData.map((batch, idx) => { const p = batch.points.find(pt => pt.age === hoveredAge); if (!p) return null; return (<div key={batch.batchId} className="flex items-center justify-between text-[10px] font-bold"><span style={{ color: lineColors[idx % lineColors.length] }}>Lote {batch.batchId}</span><span className="text-gray-900">{p.rate.toFixed(1)}%</span></div>); })}</div>)}
      </div>
    );
  };
  ...
  const renderLineChart = () => {
    const width = 900; const height = 240; const paddingLeft = 50; const paddingRight = 50; const paddingTop = 20; const paddingBottom = 40;
    ...
    return (
      <div className="relative w-full overflow-visible">
        <svg viewBox={`0 0 ${width} ${height}`} className="w-full block overflow-visible" preserveAspectRatio="xMidYMid meet">
          {[0, 0.5, 1].map(p => { const yPos = height - paddingBottom - (p * (height - paddingBottom - paddingTop)); const labelVal = Math.round(p * yAxisMax); return (<g key={p}><line x1={paddingLeft} y1={yPos} x2={width - paddingRight} y2={yPos} stroke="#f1f5f9" strokeWidth="1" strokeDasharray="4 4" /><text x={paddingLeft - 10} y={yPos + 3} textAnchor="end" className="text-[10px] fill-gray-400 font-bold">{labelVal >= 1000 ? `${(labelVal / 1000).toFixed(1)}k` : labelVal}</text></g>); })}
          {MONTHS_SHORT.map((m, i) => (<text key={m} x={getX(i)} y={height - 10} textAnchor="middle" className="text-[11px] fill-gray-400 font-black uppercase">{m}</text>))}
        ...
        {hoveredMonthIdx !== null && (<div className="absolute bg-white/95 backdrop-blur-md p-2 rounded-lg shadow-xl border border-gray-100 z-50 pointer-events-none min-w-[140px]" style={{ top: '10px', left: `${Math.min(80, Math.max(10, (getX(hoveredMonthIdx) / width) * 100))}%` }}><div className="flex items-center gap-1.5 mb-1 pb-1 border-b border-gray-50"><Calendar size={10} className="text-blue-500" /><span className="text-[9px] font-black text-gray-700 uppercase">{MONTHS[hoveredMonthIdx]}</span></div><div className="flex items-center justify-between text-[10px] font-bold"><span className="text-gray-500">Produção:</span><span className="text-gray-800">{(isSingleAviary ? (lineChartData[hoveredMonthIdx] as Record<string, number>)[activeId] : (lineChartData[hoveredMonthIdx] as Record<string, number>)['total']).toLocaleString('pt-BR')}</span></div></div>)}
      </div>
    );
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {!stats ? <div className="p-20 text-center bg-white rounded-2xl border border-dashed border-gray-200 text-gray-400 font-medium">Nenhum dado encontrado para os filtros selecionados.</div> : (<div className="space-y-6 animate-in fade-in duration-500"><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4"><KPICard title="Total Ovos" value={stats.totalEggs.toLocaleString('pt-BR')} subtext="(acumulado no período)" icon={<Egg />} color="blue" /><KPICard title="Taxa Média Postura" value={`${stats.avgLayingRate.toFixed(1)}%`} subtext="(produção/população)" icon={<TrendingUp />} color="green" /><KPICard title="Aves Vivas" value={stats.currentBirds.toLocaleString('pt-BR')} subtext="(saldo atual somado)" icon={<Bird />} color="purple" /><KPICard title="Mortalidade" value={`${stats.totalMortality} (${stats.mortalityRate.toFixed(2)}%)`} subtext="Total e Taxa no Período" icon={<AlertTriangle />} color="red" /></div><div className="grid grid-cols-1 lg:grid-cols-3 gap-6"><div className="lg:col-span-2 bg-white p-6 md:p-8 rounded-2xl border border-gray-100 shadow-sm"><div className="flex items-center gap-3 mb-8"><div className="w-2.5 h-6 bg-emerald-600 rounded-full"></div><h3 className="text-[13px] font-black text-[#1e293b] uppercase tracking-tight">QUALIDADE DOS OVOS</h3></div><div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center"><div className="relative flex justify-center py-4"><svg viewBox="0 0 100 100" className="w-40 h-40 md:w-48 md:h-48 -rotate-90"><circle cx="50" cy="50" r="40" fill="transparent" stroke="#f1f5f9" strokeWidth="12" /><SegmentedCircle stats={stats} /></svg><div className="absolute inset-0 flex flex-col items-center justify-center text-center"><p className="text-2xl md:text-3xl font-black text-gray-800">{((stats.quality.clean / (stats.quality.total || 1)) * 100).toFixed(0)}%</p><p className="text-[8px] font-black text-gray-400 uppercase tracking-widest">Limpos</p></div></div><div className="grid grid-cols-2 gap-3"><QualityBox label="Limpos" value={stats.quality.clean} total={stats.quality.total} color="emerald" /><QualityBox label="Sujos" value={stats.quality.dirty} total={stats.quality.total} color="orange" /><QualityBox label="Trincados" value={stats.quality.cracked} total={stats.quality.total} color="red" /><QualityBox label="Cama" value={stats.quality.floor} total={stats.quality.total} color="purple" /></div></div></div><div className="space-y-4"><KPICard title="Peso Ovos" value={`${stats.avgEggWeight.toFixed(1)}g`} icon={<Scale />} color="teal" subtext="" /><KPICard title="Peso Aves" value={`${stats.avgBirdWeight.toFixed(1)}g`} icon={<Weight />} color="indigo" subtext="" /><div className="bg-emerald-600 p-6 rounded-2xl shadow-lg shadow-emerald-100 flex items-center gap-4 text-white"><div className="p-3 bg-white/20 rounded-xl"><CheckCircle2 size={24} /></div><div><p className="text-[10px] font-bold uppercase tracking-widest opacity-80">Status Sanitário</p><p className="text-lg font-black">Lote Saudável</p></div></div></div></div><div className="space-y-6"><div className="bg-white p-6 md:p-8 rounded-2xl border border-gray-100 shadow-sm"><div className="flex items-center gap-3 mb-6"><div className="w-2.5 h-6 bg-blue-600 rounded-full"></div><h3 className="text-[13px] font-black text-[#1e293b] uppercase tracking-tight">DESEMPENHO DE PRODUÇÃO MENSAL</h3></div><div className="w-full">{renderLineChart()}</div></div><div className="bg-white p-6 md:p-8 rounded-2xl border border-gray-100 shadow-sm"><div className="w-full">{renderMaturityChart()}</div></div></div></div>)}
    </div>
  );
};
...
const FilterSelect = ({ value, onChange, options, customLabels }: { value: string, onChange: (v: string) => void, options: string[], customLabels?: { label: string, value: string }[] }) => (<div className="relative flex-1 min-w-[140px] md:flex-none"><select value={value} onChange={e => onChange(e.target.value)} className="appearance-none w-full bg-gray-50 border border-gray-100 text-[10px] font-black py-2.5 pl-3 pr-8 rounded-xl outline-none focus:ring-2 focus:ring-blue-100 cursor-pointer uppercase tracking-tight transition-all">{options.map(o => { const custom = customLabels?.find(cl => cl.value === o); return <option key={o} value={o}>{custom ? custom.label : o}</option>; })}</select><ChevronDown size={12} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" /></div>);
const QualityBox = ({ label, value, total, color }: { label: string, value: number, total: number, color: string }) => { const colorMap: Record<string, string> = { emerald: 'text-emerald-600 border-emerald-100 bg-emerald-50/30', orange: 'text-orange-600 border-orange-100 bg-orange-50/30', red: 'text-red-600 border-red-100 bg-red-50/30', purple: 'text-purple-600 border-purple-100 bg-purple-50/30' }; return (<div className={`p-4 rounded-xl border ${colorMap[color]} transition-transform active:scale-95`}><p className="text-[8px] font-black uppercase tracking-widest mb-1 opacity-70">{label}</p><p className="text-lg font-black">{value.toLocaleString('pt-BR')}</p><p className="text-[9px] font-bold opacity-60">{((value / (total || 1)) * 100).toFixed(1)}%</p></div>); };
const KPICard = ({ title, value, subtext, icon, color }: { title: string, value: string, subtext: string, icon: React.ReactNode, color: string }) => { const colorMap: Record<string, string> = { blue: 'bg-blue-50 text-blue-600', green: 'bg-green-50 text-green-600', purple: 'bg-purple-50 text-purple-600', red: 'bg-red-50 text-red-600', teal: 'bg-teal-50 text-teal-600', indigo: 'bg-indigo-50 text-indigo-600' }; return (<div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm flex items-center gap-4 w-full group hover:border-blue-100 transition-colors"><div className={`p-3.5 rounded-xl ${colorMap[color] || 'bg-gray-50'} transition-transform group-hover:scale-110`}>{React.cloneElement(icon as React.ReactElement<any>, { size: 24, strokeWidth: 2.5 })}</div><div className="flex-1 overflow-hidden"><p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-0.5 truncate">{title}</p><p className="text-xl font-black text-gray-800 tracking-tight">{value}</p>{subtext && <p className="text-[8px] font-bold text-gray-400 mt-0.5 truncate">{subtext}</p>}</div></div>); };
